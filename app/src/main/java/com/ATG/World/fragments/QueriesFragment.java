...
import com.ATG.World.adapters.QueriesAdapter;
 ...
 */
public class QueriesFragment extends BaseFragment implements QueriesAdapter.OnReloadClickListener, QueriesAdapter.OnItemClickListener {

    public static final int PAGE_SIZE = 7;
    public static final int GET_QRIOUS_FILTER = 6;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        queriesAdapter = new QueriesAdapter();
        queriesAdapter.setOnItemClickListener(this);
        queriesAdapter.setOnReloadClickListener(this);

        mRecyclerView.setItemAnimator(new DefaultItemAnimator());
        mRecyclerView.setAdapter(queriesAdapter);

        // Pagination
        mRecyclerView.addOnScrollListener(recyclerViewOnScrollListener);

        AtgService atgService = AtgClient.getClient().create(AtgService.class);
        Call<DashboardResponse> call = atgService.getDashboardData(GET_QRIOUS_FILTER, PAGE_START, Integer.parseInt(UserPreferenceManager.getUserId(getActivity())));
        calls.add(call);
    ...
    public void onReloadButtonClicked() {
        ...
        AtgService atgService = AtgClient.getClient().create(AtgService.class);
        Call<DashboardResponse> fetchAll = atgService.getDashboardData(GET_QRIOUS_FILTER, PAGE_START, Integer.parseInt(UserPreferenceManager.getUserId(getActivity())));
        calls.add(fetchAll);
    ...
    private void loadMoreItems() {
        ...
        AtgService atgService = AtgClient.getClient().create(AtgService.class);
        Call<DashboardResponse> call = atgService.getDashboardData(GET_QRIOUS_FILTER, mCurrentPage, Integer.parseInt(UserPreferenceManager.getUserId(getActivity())));
            ...
            if (dashboardResponse != null) {
                ...
                if (dashboardList != null) {
                    if (dashboardList.size() > 0) {
                        queriesAdapter.addAll(dashboardList);
                    }
                    if (dashboardList.size() >= PAGE_SIZE) {
                        queriesAdapter.addFooter();
                    } else {
                        isLastPage = true;
                    }
                }
    ...
    public Callback<DashboardResponse> nextFetchCallback = new Callback<DashboardResponse>() {
        ...
        public void onFailure(Call call, Throwable t) {
            ...
            if (!call.isCanceled()) {
                if (NetworkUtility.isKnownException(t)) {
                    queriesAdapter.updateFooter(QueriesAdapter.FooterType.ERROR);
                }
            }
